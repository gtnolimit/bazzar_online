	...
	Map<String, List<Item>> occurrencesCount(Path file) throws IOException {
		...
		try (ICsvDozerBeanReader beanReader = new CsvDozerBeanReader(
			...
			for (List<Item> listItems : items.values()) {
				itemDao.batchInsert(listItems, null);
			}
		}
		...
		protected Map<String, List<Item>> compute() {
			...
			try (ICsvDozerBeanReader beanReader = new CsvDozerBeanReader(
				...
				for (Entry<String, List<Item>> entry : items.entrySet()) {
					...
					List<Category> categories = itemDao.findItemParent(keys);
					if (categories == null || categories.isEmpty()) {
						...
						itemDao.createCategory(category);
					...
					for (Category category : categories) {
						...
						if (category.getSubCategory().isEmpty()) {
							...
							category.getSubCategory().add(subCategory);
							itemDao.createCategory(category);
						} else if (category.getSubCategory().iterator().next()
						        .getProduct().isEmpty()) {
							product = new Product();
							product.setAttribute(keys.get(2));
							category.getSubCategory().iterator().next()
							        .getProduct().add(product);
							itemDao.createCategory(category);
						} else {
							product = category.getSubCategory().iterator()
							        .next().getProduct().iterator().next();
						}

						itemDao.batchInsert(entry.getValue(), product);
					}
				}
			} catch (IOException e) {
				throw new RuntimeException(e.getMessage(), e.getCause());
